...
def populate_search_table(conn):
    ...
    with conn.cursor() as cur:
        ...
        print("Apagando dados da tabela de busca e reiniciando a sequência.")
        cur.execute("DELETE FROM ceps_search;")
        cur.execute("ALTER SEQUENCE ceps_search_id_seq RESTART WITH 1;")
        conn.commit()
        
        print("Populando tabela de busca. Isso pode levar alguns minutos...")
        cur.execute("""
        ...
        """)
    conn.commit()
    print("Tabela de busca populada com sucesso!")
